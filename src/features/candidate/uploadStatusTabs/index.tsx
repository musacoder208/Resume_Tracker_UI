    ...
    <div className="flex items-center gap-3 py-3.5">
      ...
      {tab === 'success' && (
        <input
          ...
          style={checkboxConfig.accentColor != null ? { accentColor: checkboxConfig.accentColor } : undefined}
        />
      )}
...
export function UploadStatusTabs({
  ...
  //   - complete (ready + score_pending) → pre-selected
  //   - incomplete / duplicate   → unchecked
  useEffect(() => {
    if (data == null) return;

    const initial = new Set<number>();
    data.complete.forEach((c) => { initial.add(c.candidateId); });
  ...
  const hasSelectedPending = (data?.complete ?? []).some(
    (c) => c.statusCode === 'score_pending' && selectedIds.has(c.candidateId),
  );
    ...
    <div className="flex flex-col rounded-xl border border-border bg-surface shadow-sm">
      ...
      <div className="flex items-center justify-end border-t border-border px-5 py-4">
        <Button
          ...
          onClick={() => { onSaveAndScore(Array.from(selectedIds)); }}
        >
