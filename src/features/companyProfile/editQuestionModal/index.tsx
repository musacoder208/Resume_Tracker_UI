...
import { useUpdateAnswerMutation } from '../api/companyProfile.api';
import type { EditNextQuestion, EditStep } from '../types/companyProfile.types';
...
function AnswerInput({ question, isSubmitting, onSubmit }: AnswerInputProps): JSX.Element {
  ...
  const allSelected =
    allowedValues.length > 0 && selectedMulti.length === allowedValues.length;
...
export function EditQuestionModal({
  ...
  const [updateAnswer, { isLoading: isSubmitting }] = useUpdateAnswerMutation();
  ...
  // Sync if parent re-opens modal with a new question
  useEffect(() => {
    if (open) {
  ...
  async function handleAnswerSubmit(answer: string): Promise<void> {
    try {
      const data = await updateAnswer({ answer }).unwrap();
        ...
        {currentStep === 'final_confirm' && (
          <div className="rounded-xl border border-warning/40 bg-warning/5 p-5">
            <div className="mb-4 flex items-start gap-3">
              ...
              <p className="text-sm font-medium text-text">{currentQuestion.text}</p>
            </div>
